...
TEST_F(test_parse_class, class_statement_with_methods) {
  {
    ...
    EXPECT_THAT(p.property_declarations, ElementsAre(u8"eatMuffins"));
...

  {
    test_parser p(
        ...
        u8"}");
    p.parse_and_visit_statement();
    EXPECT_THAT(p.property_declarations, ElementsAre(u8"length", u8"length"));
  }
...
TEST_F(test_parse_class, class_statement_with_fields) {
  {
    ...
    EXPECT_THAT(p.property_declarations, ElementsAre(u8"banana"));
  }
...

  {
    ...
    EXPECT_THAT(p.visits,
                ElementsAre("visit_enter_class_scope",       //
                            ...
                            "visit_variable_declaration"));  //
    EXPECT_THAT(p.property_declarations, ElementsAre(std::nullopt));
    EXPECT_THAT(p.variable_uses, ElementsAre(u8"pi"));
  }
...

  {
    ...
    EXPECT_THAT(p.variable_uses, ElementsAre(u8"x", u8"y", u8"init"));
  }
...
TEST_F(test_parse_class,
  ...
  for (string8_view keyword : keywords) {
    class_declarations.emplace_back(concat(keyword, u8"() {}"));
  }
  for (const string8& second_member : class_declarations) {
    ...
    for (string8_view first_member : {u8"3.14", u8"'bananas'", u8"[expr]"}) {
      padded_string code(
          concat(u8"class C { ", first_member, u8"\n", second_member, u8" }"));
      SCOPED_TRACE(code);
      test_parser p(code.string_view());
      p.parse_and_visit_statement();
      EXPECT_THAT(p.property_declarations,
  ...
  for (string8_view keyword : strict_reserved_keywords) {
    ...
    for (string8_view prefix :
         {u8""_sv, u8"*"_sv, u8"async"_sv, u8"async *"_sv, u8"get"_sv,
          ...
          u8"static async *"_sv, u8"static get"_sv, u8"static set"_sv}) {
      padded_string code(
          concat(u8"class C { ", prefix, u8" ", property, u8"(){} }"));
      SCOPED_TRACE(code);
      test_parser p(code.string_view());
      p.parse_and_visit_statement();
      EXPECT_THAT(p.property_declarations, ElementsAre(keyword));
...
TEST_F(test_parse_class, typescript_constructor_this_parameter_is_disallowed) {
  {
    ...
    EXPECT_THAT(
        p.errors,
        ElementsAre(DIAG_TYPE_OFFSETS(
            p.code, diag_this_parameter_not_allowed_in_constructors,  //
            this_keyword, strlen(u8"class C { constructor("), u8"this")));
  }

  {
    test_parser p(u8"class C { constructor() { let o = { method(this) {} }; function f(this) {}}}"_sv,
                  typescript_options);
    p.parse_and_visit_statement();
    EXPECT_THAT(p.visits,
                ElementsAre("visit_enter_class_scope",
  ...
  {
    test_parser p(u8"class C { constructor() { class F { constructor() { let o = { method(this) {}}}}}}"_sv,
                  typescript_options);
    p.parse_and_visit_statement();
    EXPECT_THAT(p.visits, ElementsAre("visit_enter_class_scope",
                                      "visit_enter_class_scope_body",
...
TEST_F(test_parse_class, class_expression_body_is_visited_first_in_expression) {
...

  {
    ...
    EXPECT_THAT(p.variable_uses, ElementsAre(u8"inside"));
